...
def executeLogisticRegression():
    global classifier
    global LogisticRegression_acc
    text.delete('1.0', END)
    cls = LogisticRegression(random_state=0).fit(X, Y) 
    cls.fit(X_train, y_train)
    predict = cls.predict(X_test)
    LogisticRegression_acc = accuracy_score(y_test,predict)*100
    classifier = cls
    text.insert(END,"Logistic : "+str(LogisticRegression_acc)+"\n")
...
def graph():
    height = [svm_acc, kmeans_acc,LogisticRegression_acc]
    bars = ('SVM Accuracy','KMeans Accuracy','Logistic Accuracy')
...
LogisticButton = Button(main, text="Execute Logistic Algorithm", command=executeLogisticRegression)
LogisticButton.place(x=750,y=600)
LogisticButton.config(font=font1) 
